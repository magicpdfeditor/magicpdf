...
export default function HtmlToPdfPage() {
  ...
  const handleConvert = async () => {
    if (!htmlContent && !file) {
      setError('Please enter HTML content or select a file');
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="max-w-4xl mx-auto px-4 py-20">
        <div className="bg-white rounded-lg shadow-xl p-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">HTML to PDF</h1>
          <p className="text-gray-600 mb-8">Convert HTML content or files to PDF</p>

          <div className="mb-8">
            <label className="block text-lg font-semibold text-gray-700 mb-4">Enter HTML Content</label>
            <textarea
              value={htmlContent}
              onChange={(e) => setHtmlContent(e.target.value)}
              placeholder="<h1>My PDF</h1><p>Your content here...</p>"
              className="w-full h-32 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600"
            />
          </div>

          <div className="mb-8 text-center text-gray-600">OR</div>

          <div className="mb-8">
            <label className="block text-lg font-semibold text-gray-700 mb-4">Select HTML File</label>
          ...
          <button
            ...
            disabled={(htmlContent.trim() === '' && !file) || uploading}
