...
export default function RedactPdfPage() {
  ...
  const handleRedact = async () => {
    ...
    try {
      ...
      formData.append('redactText', redactionText);
      ...
      if (!response.ok) throw new Error('Failed to redact');
      ...
      setError('Error redacting PDF. Please try again.');
      console.error(err);
    } finally {
      setUploading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="max-w-4xl mx-auto px-4 py-20">
        <div className="bg-white rounded-lg shadow-xl p-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Redact PDF</h1>
          <p className="text-gray-600 mb-2">Remove sensitive information from PDFs</p>
          <p className="text-gray-500 mb-8">This free PDF redaction tool lets you permanently remove sensitive text and information from your documents before sharing them.</p>
          ...
          <div className="mb-8">
            <label className="block text-lg font-semibold text-gray-700 mb-4">Text to Redact</label>
            <textarea
              value={redactionText}
              onChange={(e) => setRedactionText(e.target.value)}
              placeholder="Enter text you want to redact"
              className="w-full h-24 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600"
            />
          </div>
          ...
          >
            {uploading ? 'Redacting...' : 'Redact PDF'}
          </button>
        </div>
      </div>
